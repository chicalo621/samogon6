...
float max_flow_ml_h = 40000.0;       // Макс. продуктивність, мл/год
float target_volume = 0.0;           // Цільовий ліміт автостопу, мл (0 = вимкнено)
//unsigned long vol_last_ms = 0UL;     // Таймер підрахунку об'єму
// --- Stepper (майбутній режим controlMode == 2) ---
float stepper_ml_per_rev = 10.0;            // Калібрування: мл за 1 оберт
const float stepper_steps_per_rev = 1600.0; // NEMA17, 1/8 мікрокрок = 200*8
volatile uint32_t stepPeriodUs = 0;         // Період STEP імпульсів у мкс
volatile uint32_t stepPulseCount = 0;       // Лічильник STEP імпульсів (для діагностики)
// ─── Аварійні прапори ────────────────────────────────────────────────────────
...
volatile int  tempInt2   = 0;   // ЗНАЧЕННЯ ШІМ КЛАПАНА (0..1023):
...
MenuItem menuItems[MENU_ITEMS_COUNT] = {
  ...
  { 0, &controlMode,                TYPE_INT,   1,    0,    2,    nullptr, MENU_SETUP }, // Klapan/motor: 0=реле, 1=ШІМ
...
// --- НОВА ЛОГІКА: перевірка EEPROM проти шини + обнулення відсутніх + запис нових у вільні слоти ---
 ...
 Timer1.initialize(100);  // Interrupt кожні 100 мкс для ШІМ
Timer1.attachInterrupt(extendmotor);
...
// ═══════════════════════════════════════════════════════════════════════════
void updateStepperRate() {
  if (controlMode != 2 || tempInt2 <= 0 || stepper_ml_per_rev <= 0.0f) {
    stepPeriodUs = 0;
    return;
  }

  // Поточна потрібна продуктивність відносно 1023 = 100%
  double flow_now_ml_h = ((double)max_flow_ml_h * (double)tempInt2) / 1023.0;

  if (flow_now_ml_h <= 0.0) {
    stepPeriodUs = 0;
    return;
  }

  // Оберти на годину
  double rev_per_h = flow_now_ml_h / (double)stepper_ml_per_rev;

  // Кроки на секунду
  double steps_per_s = (rev_per_h * (double)stepper_steps_per_rev) / 3600.0;

  if (steps_per_s < 0.001) {
    stepPeriodUs = 0;
    return;
  }
...
void extendmotor() {
  ...
  if (controlMode == 1) {
    ...
    uint16_t maxCount = pwmPeriodUs / isrTickUs;
    if (maxCount == 0) maxCount = 1;

    uint16_t dutyCount = ((uint32_t)tempInt2 * maxCount) / 1023;

    pwmCounter++;
    if (pwmCounter >= maxCount) pwmCounter = 0;

    digitalWrite(VALVE_RELAY_DIRECT_PIN, (pwmCounter < dutyCount) ? HIGH : LOW);
    return;
  }

  // ── РЕЖИМ 2: КРОКОВИЙ ДВИГУН / STEP ────────────────────────────
  if (controlMode == 2) {
    static uint32_t stepElapsedUs = 0;
    static bool stepPinHigh = false;
    ...
    stepElapsedUs += isrTickUs;

    if (!stepPinHigh) {
      // Чекаємо моменту нового STEP імпульсу
      if (stepElapsedUs >= stepPeriodUs) {
        digitalWrite(VALVE_RELAY_DIRECT_PIN, HIGH); // STEP HIGH
        stepPinHigh = true;
        stepElapsedUs = 0;
        stepPulseCount++;
      }
    } else {
      // Тримаємо HIGH рівно 1 тик ISR = 100 мкс, потім LOW
      digitalWrite(VALVE_RELAY_DIRECT_PIN, LOW); // STEP LOW
      stepPinHigh = false;
    }
